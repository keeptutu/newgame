...
class Word():
    def __init__(self):
        ...
        self.test13 = make_right_word(pygame, '风暴峭壁', 24)
...
def show_belong(blocks):
    for i in blocks:
        if 0 < i.num < 8:
            ...
            screen.screen.blit(eval('belong' + str(i.belong)), eval('block_rect' + str(i.num) + '.topleft'))
        if i.belong != 0 and 0 < i.num < 8:
            screen.screen.blit(i.show_passmoney,(eval('block_rect' + str(i.num) + '.topleft[0]+46'),eval('block_rect' + str(i.num) + '.topright[1]+10')))
        elif i.belong != 0 and 8 < i.num < 14:
            screen.screen.blit(i.show_passmoney,(eval('block_rect' + str(i.num) + '.topleft[0]+46'),eval('block_rect' + str(i.num) + '.topright[1]+30')))
        elif i.belong != 0 and 14 < i.num < 22:
            screen.screen.blit(i.show_passmoney,(eval('block_rect' + str(i.num) + '.topleft[0]'),eval('block_rect' + str(i.num) + '.topright[1]+80')))
        elif i.belong != 0 and 22 < i.num < 27:
            screen.screen.blit(i.show_passmoney,(eval('block_rect' + str(i.num) + '.topleft[0]+46'),eval('block_rect' + str(i.num) + '.topright[1]+10')))
...
# 房屋建筑显示图片预载
l01 = pygame.image.load(l1_filename).convert_alpha()
l02 = pygame.image.load(l2_filename).convert_alpha()
l03 = pygame.image.load(l3_filename).convert_alpha()
...
tou = Tou()
# 单人游戏模式下的实例
p1 = Player('玩家', 100000, 0, p1p,1,mode='human')
p2 = Player('电脑1', 100000, 2, p2p,2)
p3 = Player('电脑2', 100000, 4, p3p,3)
p4 = Player('电脑3', 100000, 6, p4p,4)
# 创建信息框实例
info = Info()
# 建立人物循环
player = p1
...
while n == 2:
    ...
    for i in range(28):
        if i not in [0, 8, 14, 22]:
            ...
            if eval('block_rect' + str(i) + '.collidepoint(mouse.x,mouse.y)'):
